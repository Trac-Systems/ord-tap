...
impl InscriptionUpdater<'_, '_> {
  ...
  pub(crate) fn verify_privilege_signature_with_msg(
    ...
    if let Some(ptr) = link_ptr {
      if let Some(bytes) = self.tap_db.get(ptr.as_bytes()).ok().flatten() {
        // Try typed record first
        if let Ok(link_rec) = ciborium::de::from_reader::<self::records::PrivilegeAuthCreateRecord, _>(std::io::Cursor::new(&bytes)) {
          // Verify authority signature against JSON.stringify(auth)+slt
          let sig = &link_rec.sig;
          let r2s = sig.get("r").and_then(|v| v.as_str()).unwrap_or("");
          let s2s = sig.get("s").and_then(|v| v.as_str()).unwrap_or("");
          let v2i = sig.get("v").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
          ...
          if !cancelled && ok2 && pubkey_hex == auth_pk_hex && prv_addr == address { link_ok = true; }
        } else if let Ok(link_json_str) = ciborium::de::from_reader::<String, _>(std::io::Cursor::new(&bytes)) {
          // Backward compatibility: JSON string payload
          if let Ok(link_val) = serde_json::from_str::<serde_json::Value>(&link_json_str) {
            if let (Some(sig), Some(hash_hex), Some(auth_obj), Some(slt), Some(link_ins)) = (
              ...
              let r2 = sig.get("r").and_then(|v| v.as_str()).unwrap_or(""); let s2 = sig.get("s").and_then(|v| v.as_str()).unwrap_or(""); let v2 = sig.get("v").and_then(|v| v.as_i64()).unwrap_or(0) as i32;
              ...
              let auth_pk = secp.recover_ecdsa(&rmsg2, &rsig2).ok()?;
              let auth_pk_hex = hex::encode(auth_pk.serialize_uncompressed());
              // verify auth signature against JSON.stringify(auth)+slt
