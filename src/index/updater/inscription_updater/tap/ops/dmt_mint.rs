...
pub(crate) struct DmtMintMetaRecord {
  ...
  // so old local indexes with object-shaped metadata can still be read.
...
impl InscriptionUpdater<'_, '_> {
  ...
  pub(crate) fn index_dmt_mint(
    ...
    let op = json_val
      ...
      .unwrap_or("")
      .to_lowercase();
    let tick_user = json_val
      .get("tick")
      .and_then(|v| v.as_str())
      .unwrap_or("")
      .to_string();
    let blk_v = json_val.get("blk");
    if p != "tap" || op != "dmt-mint" || tick_user.is_empty() || blk_v.is_none() {
      return;
    }
    ...
    let ok_len = if !full {
      vis_len == 3 || (vis_len >= 5 && vis_len <= 32)
    } else {
      vis_len > 0 && vis_len <= 32
    };
    if !ok_len {
      return;
    }
    ...
    let (parsed_blk_i64, blk_js_str) = match Self::js_parse_int_repr(blk_v.unwrap()) {
      Some(t) => t,
      None => return,
    };
    ...
    if self.tap_feature_enabled(TapFeature::DmtParseintActivation)
      && parsed_blk_i64.to_string() != blk_js_str
    {
      return;
    }
    ...
    if let Some(dta_val) = json_val.get("dta") {
      if let Some(ds) = dta_val.as_str() {
        if ds.as_bytes().len() > 512 {
          return;
        }
        ins_data = Some(ds.to_string());
      }
    }

    if self
      .tap_get::<String>(&format!("dmt-blk/{}/{}", tick_effective_lower, parsed_blk))
      .ok()
      .flatten()
      .is_some()
    {
    ...
    let dep_json = if let Some(dep_val) = json_val.get("dep") {
    ...
    if dep_json.is_none() && !self.tap_feature_enabled(TapFeature::FullTicker) {
      return;
    }
    ...
    if let Some(prj) = deployed.prj.clone() {
      if let Some(project_tick) = self
        .tap_get::<String>(&format!("dmt-di/{}", prj))
        .ok()
        .flatten()
      {
        let ptk = Self::json_stringify_lower(&project_tick);
        if let Some(project_dep) = self
          .tap_get::<DeployRecord>(&format!("d/{}", ptk))
          .ok()
          .flatten()
        {
          if project_dep.dmt {
        ...
        if let Some(bm_ptr) = self
          .tap_get::<String>(&format!("bmh/{}", prj))
          .ok()
          .flatten()
        {
          if bm_ptr == "bm/0" {
        ...
        if is_unat {
          if let Ok(Some(bytes)) = self.tap_db.get(format!("dmtmh/{}", parent_id).as_bytes()) {
            if let Ok(unat_val) = serde_json::from_slice::<serde_json::Value>(&bytes) {
              if let Some(pt) = &project_ticker {
                if unat_val.get("tick").and_then(|v| v.as_str()) == Some(pt.as_str()) {
          ...
          if self
            .tap_get::<String>(&format!("bmh/{}", parent_id))
            .ok()
            .flatten()
            .is_some()
          {
    ...
    let Some(dep_str) = the_dep else {
      return;
    };
    if deployed.ins != dep_str {
      return;
    }
    if !Self::writer_loose_inscription_id_syntax(&dep_str) {
      return;
    }
    if !self.ordinal_available(&dep_str, index) {
      return;
    }
    ...
    let Some(elem_ins) = deployed.elem.clone() else {
      return;
    };
    let Some(elem_name) = self
      .tap_get::<String>(&format!("dmt-{}", elem_ins))
      .ok()
      .flatten()
    else {
      return;
    ...
    match elem.fld {
      4 => {
        ...
        if let Some(pat) = &elem.pat {
          if deployed.dt.as_deref() != Some("n") {
            return;
          }
          if let Some(cnt) = js_count_global_matches(pat, &c_val) {
            amount = cnt as i128;
          } else {
            return;
          }
        } else {
          amount = match Self::js_bigint_string_to_i128(&blk_js_str) {
            Some(v) => v,
            None => return,
          };
        }
      ...
      10 => {
        ...
        let hdr: TapHeaderSnapshot = match self
          .tap_db
          .get(format!("hdr/{}", parsed_blk).as_bytes())
          .ok()
          .flatten()
          .and_then(|b| {
            ciborium::de::from_reader::<TapHeaderSnapshot, _>(std::io::Cursor::new(b)).ok()
          }) {
          Some(h) => h,
          None => return,
        };
    ...
    if !fail {
      ...
      if self
        .tap_get::<String>(&format!("he/{}/{}", owner_address, tick_key))
        .ok()
        .flatten()
        .is_none()
      {
        let _ = self.tap_put(
          &format!("he/{}/{}", owner_address, tick_key),
          &"".to_string(),
        );
    ...
    let elem_string = Self::js_json_stringify(&elem_json);

    // Holder record and pointers are written only on successful mint
    if !fail {
      // Parity: writer stores JSON.stringify(elem), so the field value is a JSON string.
      ...
      let mut cur_len: u64 = self
        .tap_get::<String>(&len_key)
        .ok()
        .flatten()
      ...
      let _ = self.tap_set_list_record(
        ...
        &format!("txi/dmt-md/{}", txs),
        &ptr,
      );
      let _ = self.tap_set_list_record(
        &format!("txt/dmt-md/{}/{}", tick_key, txs),
        &format!("txti/dmt-md/{}/{}", tick_key, txs),
        &ptr,
      );
      let _ = self.tap_set_list_record(
        &format!("blck/dmt-md/{}", self.height),
        &format!("blcki/dmt-md/{}", self.height),
        &ptr,
      );
      let _ = self.tap_set_list_record(
        &format!("blckt/dmt-md/{}/{}", tick_key, self.height),
        &format!("blckti/dmt-md/{}/{}", tick_key, self.height),
        &ptr,
      );
      // Wallet historic list (parity)
      let _ = self.tap_set_list_record(
        &format!("dmtmwl/{}", owner_address),
        &format!("dmtmwli/{}", owner_address),
        &inscription_id.to_string(),
      );
      // Mark block consumed for this tick (one per dmt block) — written last (parity with writer)
      let _ = self.tap_put(
        &format!("dmt-blk/{}/{}", tick_effective_lower, parsed_blk),
        &"".to_string(),
      );
    }

    if !Self::trim_js_whitespace(owner_address).eq("-") && !fail {
      let meta = DmtMintMetaRecord {
        tick: tick_effective_lower.clone(),
        elem: serde_json::Value::String(elem_string),
        dmtblck: parsed_blk,
        blckdrp: is_blockdrop,
        dep: dep_str.clone(),
      ...
      if self
        .tap_get::<String>(&format!("ato/{}/{}", owner_address, tick_key))
        .ok()
        .flatten()
        .is_none()
      {
        ...
        let _ = self.tap_put(
          &format!("ato/{}/{}", owner_address, tick_key),
          &"".to_string(),
        );
      }
    ...
    let _ = self.tap_set_list_record(
      &format!("aml/{}/{}", owner_address, tick_key),
      &format!("amli/{}/{}", owner_address, tick_key),
      &data_json,
    );
    let _ = self.tap_set_list_record(
      &format!("fml/{}", tick_key),
    ...
    if let Ok(list_len) = self.tap_set_list_record("sfml", "sfmli", &super_json) {
      ...
      let _ = self.tap_set_list_record(
        &format!("txt/mnt/{}/{}", tick_key, txs),
        &format!("txti/mnt/{}/{}", tick_key, txs),
        &ptr,
      );
      let _ = self.tap_set_list_record(
    ...
    if let Some(comp) = used_compact_sig {
      let _ = self.tap_put(&format!("prah/{}", comp), &"".to_string());
    }
  }
