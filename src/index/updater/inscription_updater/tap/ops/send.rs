...
impl InscriptionUpdater<'_, '_> {
  pub(crate) fn index_token_send_created(
    ...
    let op = json_val
      ...
      .unwrap_or("")
      .to_lowercase();
    if p != "tap" || op != "token-send" {
      return;
    }
    let mut items = match json_val.get("items").and_then(|v| v.as_array()).cloned() {
    ...
    for it in items.iter_mut() {
      let tick = match it.get("tick").and_then(|v| v.as_str()) {
        Some(t) => t.to_string(),
        None => return,
      };
      let addr_raw = match it.get("address").and_then(|v| v.as_str()) {
        Some(a) => a,
        None => return,
      };
      if it.get("amt").is_none() {
        return;
      }
      ...
      if let Some(d) = it.get("dta").and_then(|v| v.as_str()) {
        if d.as_bytes().len() > 512 {
          return;
        }
      }

      if let Some(addr_field) = it.get_mut("address") {
        *addr_field = serde_json::Value::String(addr_norm);
      }
    }

    let mut json_norm = json_val.clone();
    if let Some(items_val) = json_norm.get_mut("items") {
      *items_val = serde_json::Value::Array(items);
    }
    ...
    if let Ok(list_len) = self.tap_set_list_record("al", "ali", &acc) {
      ...
      let _ = self.tap_set_list_record(
        &format!("blck/a-snd/{}", self.height),
        &format!("blcki/a-snd/{}", self.height),
        &ptr,
      );
    }
  }
  ...
  pub(crate) fn index_token_send_executed(
    ...
    if acc.op.to_lowercase() != "token-send" {
      return;
    }
    // START MINER-REWARD-SHIELD
    if self.tap_is_dmt_reward_address(owner_address) {
    ...
    for item in items.iter() {
      ...
      if let Some(d) = item.get("dta").and_then(|v| v.as_str()) {
        if d.as_bytes().len() > 512 {
          continue;
        }
        ins_data = Some(d.to_string());
      }
      let Some(tick_str) = item.get("tick").and_then(|v| v.as_str()) else {
        continue;
      };
      let Some(receiver) = item.get("address").and_then(|v| v.as_str()) else {
        continue;
      };
      let Some(amt_raw) = item.get("amt") else {
        continue;
      };
